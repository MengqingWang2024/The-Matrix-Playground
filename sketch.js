...
function draw() {
  ...
  pop();
    ...
    line(370,windowHeight-50,windowWidth-20,windowHeight-50);
    //rect(20,20,280,200);
    stroke(200);
    strokeWeight(0.25);
    rect(20,20,330,158);
  ...
  // 绘制虚线
  let dashLength = 2; // 设置虚线的长度
  for (let i = startX + dashLength / 2; i <= endX; i += dashLength * 2) {
    ...
    //段落文字
    push();
    textAlign(LEFT, TOP); // 文本左对齐
    // 绘制标题
    textSize(21); // 标题字号
  drawTriangle(35, 80); // 绘制三角形
  text("Quantum Wave Generator", 50, 75);
  ...
  // 第一行
  drawTriangle2(50, 100); // 绘制三角形
  fill(120); 
  text("{I}:Enter any code", 65, 100);

  // 第二行
  drawTriangle2(50, 115); // 绘制三角形
  fill(120); 
  text("{II}:Use sliders to control the waveform", 65, 115);

  // 第三行
  drawTriangle2(50, 130); // 绘制三角形
  fill(120); 
  text("{III}:Choose blue or red pills", 65, 130);
    // 第四行
    drawTriangle2(50, 145); // 绘制三角形
    fill(120); 
    text("{IV}:Try moving the mouse on the canva", 65, 145);
...
function drawTriangle(x, y) {
  fill(255); // 设置填充颜色为黑色
  triangle(x, y, x + 7, y + 7, x, y + 14); // 绘制实心三角形
}

function drawTriangle2(x, y) {
  fill(120); // 设置填充颜色为黑色
  triangle(x, y, x + 6, y + 6, x, y + 12); // 绘制实心三角形
}
  ...
  if (distortionLayer) {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        // 在整个图层内应用干扰效果
        if (x < windowWidth-20 && x>370 && y < windowHeight-50 && y>20){
